...
Graph DB & AWS Neptune (고도화 단계) & 추천 그래프(목적지·축제·테마·인접 도시·이동 관계)를 관리한다. PoC/Prod 1차는 비용을 고려해 \texttt{neptune\_\allowbreak{}alternative.\allowbreak{}md} 대체 아키텍처를 적용한다. \\
...
{\scriptsize Dynamo\allowbreak{}DB} & 비정형 실행 상태, 이벤트 로그, TTL 로그, 수집 정규화 결과 & Agent run, async job, 사용자 행동 이벤트, API 로그, 운영 trace, City/Attraction/Festival/VisitorStatistics 정규화 문서 \\
...
\toprule
\multicolumn{1}{>{\centering\arraybackslash}m{0.264\linewidth}}{\textbf{대상}} & \multicolumn{1}{>{\centering\arraybackslash}m{0.264\linewidth}}{\textbf{권고·잠정 보존 기간(TTL)}} & \multicolumn{1}{>{\centering\arraybackslash}m{0.391\linewidth}}{\textbf{비고}} \\
\midrule
\endhead
...
\texttt{lovv\_\allowbreak{}festival\_\allowbreak{}verify\_\allowbreak{}cache} & 상태별 TTL: \texttt{confirmed} 30일, \texttt{tentative} 7일, \texttt{unknown·outdated} 1일 & 기존 확정값 \\
