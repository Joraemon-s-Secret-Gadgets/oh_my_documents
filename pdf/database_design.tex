...
\begin{small}
\begin{longtable}{@{}>{\centering\arraybackslash}m{0.131\linewidth}>{\RaggedRight\arraybackslash}m{0.253\linewidth}>{\RaggedRight\arraybackslash}m{0.535\linewidth}@{}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}m{0.131\linewidth}}{\textbf{항목}} & \multicolumn{1}{>{\centering\arraybackslash}m{0.253\linewidth}}{\textbf{결정}} & \multicolumn{1}{>{\centering\arraybackslash}m{0.535\linewidth}}{\textbf{비고}} \\
\midrule
\endhead
...
Graph DB & AWS Neptune (고도화 단계) & 목적지, 축제, 테마, 인접 도시, 이동 관계처럼 다단계 관계 탐색이 필요한 추천 그래프를 관리한다. 단, Neptune은 상시 가동 비용(최소 1 NCU 약 월 \$80+)이 높아 PoC/Prod 1차는 \texttt{neptune\_\allowbreak{}alternative.\allowbreak{}md}의 대체 아키텍처를 적용하고, 고도화 단계에서 도입을 결정한다. \\
...
\begin{small}
\begin{longtable}{@{}>{\centering\arraybackslash}m{0.150\linewidth}>{\RaggedRight\arraybackslash}m{0.357\linewidth}>{\RaggedRight\arraybackslash}m{0.413\linewidth}@{}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}m{0.150\linewidth}}{\textbf{저장소}} & \multicolumn{1}{>{\centering\arraybackslash}m{0.357\linewidth}}{\textbf{책임}} & \multicolumn{1}{>{\centering\arraybackslash}m{0.413\linewidth}}{\textbf{주요 데이터}} \\
\midrule
\endhead
MySQL 8 LTS & 서비스 원장, 트랜잭션, 조회 기준 데이터 & 사용자 계정, 소셜 계정, 저장 일정, 일정 항목, 일정 반응 \\
{\scriptsize Dynamo\allowbreak{}DB} & 비정형 실행 상태, 이벤트 로그, TTL 로그, 수집 정규화 결과 & Agent 실행 상태, async job, 사용자 행동 이벤트, API 호출 로그, plan reaction clickstream, admin operation trace, City/Attraction/Festival/VisitorStatistics 정규화 문서 \\
S3 vector index & 의미 검색 인덱스 & 목적지·축제·관광지 문서 chunk, embedding, source reference, metadata filter \\
AWS Neptune & 그래프 관계 인덱스 (상시 비용 발생, 고도화 단계 도입) & 도시-테마, 도시-축제, 도시-인접 도시, 일정 항목-장소 후보 관계. PoC/Prod 1차는 \texttt{neptune\_\allowbreak{}alternative.\allowbreak{}md} 대체 아키텍처 적용 \\
...
\DocNeedspace{3\baselineskip}
또한 데이터 수집 계획의 S3 Raw Bucket -> Lambda 전처리 결과는 City, Attraction, Festival, VisitorStatistics 단위의 정규화 문서로 DynamoDB에 적재한다.\newline 이 정규화 문서는 검색·추천 후보 생성의 빠른 조회용이며, 사용자가 저장한 일정은 MySQL의 \texttt{itineraries.\allowbreak{}preference\_\allowbreak{}snapshot}과 \texttt{itinerary\_\allowbreak{}items}에 스냅샷으로 남긴다.
...
\DocNeedspace{3\baselineskip}
아래 테이블은 DynamoDB에 둘 항목을 운영 추적, 검증 캐시, 비동기 작업, API 로그, 수집 정규화 문서로 나눈 것이다.\newline TTL이 필요한 로그성 테이블은 보존 기간 이후 자동 삭제를 전제로 하고, 수집 정규화 테이블은 S3 Raw 원본을 기준으로 재생성 가능한 조회 모델로 관리한다.


\DocNeedspace{20\baselineskip}
\begin{footnotesize}
\begin{longtable}{@{}>{\centering\arraybackslash}m{0.145\linewidth}>{\RaggedRight\arraybackslash}m{0.207\linewidth}>{\RaggedRight\arraybackslash}m{0.190\linewidth}>{\RaggedRight\arraybackslash}m{0.207\linewidth}>{\RaggedRight\arraybackslash}m{0.130\linewidth}@{}}
\toprule
...
\midrule
\endhead
목적지 & DynamoDB 정규화 문서, S3 Raw 원본 & \texttt{destination\_\allowbreak{}id}, \texttt{country}, \texttt{region}, \texttt{themes}, \texttt{recommended\_\allowbreak{}months} \\
축제 & DynamoDB 정규화 문서, S3 Raw 원본 & \texttt{festival\_\allowbreak{}id}, \texttt{destination\_\allowbreak{}id}, \texttt{start\_\allowbreak{}date}, \texttt{end\_\allowbreak{}date}, \texttt{season\_\allowbreak{}tags} \\
...
관광지·체험 & DynamoDB 정규화 문서, S3 Raw 원본 & \texttt{content\_\allowbreak{}id}, \texttt{destination\_\allowbreak{}id}, \texttt{content\_\allowbreak{}type}, \texttt{theme\_\allowbreak{}tags} \\
출처 문서 & object storage & \texttt{source\_\allowbreak{}id}, \texttt{source\_\allowbreak{}url}, \texttt{collected\_\allowbreak{}at}, \texttt{license\_\allowbreak{}type} \\
\bottomrule
\end{longtable}
\end{small}
...
대체 설계는 \texttt{neptune\_\allowbreak{}alternative.\allowbreak{}md}를 따른다. 본 3.5절/4.6절은 고도화 단계 Neptune 도입 시의 목표 모델이다.\\
\end{quote}


\DocNeedspace{16\baselineskip}
\begin{footnotesize}
\begin{longtable}{@{}>{\centering\arraybackslash}m{0.108\linewidth}>{\centering\arraybackslash}m{0.264\linewidth}>{\RaggedRight\arraybackslash}m{0.352\linewidth}>{\RaggedRight\arraybackslash}m{0.176\linewidth}@{}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}m{0.108\linewidth}}{\textbf{그래프 요소}} & \multicolumn{1}{>{\centering\arraybackslash}m{0.264\linewidth}}{\textbf{라벨/관계}} & \multicolumn{1}{>{\centering\arraybackslash}m{0.352\linewidth}}{\textbf{주요 속성}} & \multicolumn{1}{>{\centering\arraybackslash}m{0.176\linewidth}}{\textbf{용도}} \\
\midrule
\endhead
...
관계 & \texttt{HAS\_\allowbreak{}THEME}, \texttt{HOSTS\_\allowbreak{}FESTIVAL}, \texttt{NEAR\_\allowbreak{}CITY}, \texttt{HAS\_\allowbreak{}PLACE}, \texttt{SEASONAL\_\allowbreak{}FIT} & \texttt{weight}, \texttt{confidence}, \texttt{source\_\allowbreak{}id}, \texttt{updated\_\allowbreak{}at} & 다단계 후보 확장과 그래프 기반 재랭킹 \\
\bottomrule
\end{longtable}
\end{footnotesize}
...
\DocNeedspace{16\baselineskip}
\begin{footnotesize}
\begin{longtable}{@{}>{\centering\arraybackslash}m{0.130\linewidth}>{\RaggedRight\arraybackslash}m{0.290\linewidth}>{\RaggedRight\arraybackslash}m{0.320\linewidth}>{\RaggedRight\arraybackslash}m{0.160\linewidth}@{}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}m{0.130\linewidth}}{\textbf{API 필드}} & \multicolumn{1}{>{\centering\arraybackslash}m{0.290\linewidth}}{\textbf{MySQL 기준}} & \multicolumn{1}{>{\centering\arraybackslash}m{0.320\linewidth}}{\textbf{DynamoDB/S3 vector/Neptune 기준}} & \multicolumn{1}{>{\centering\arraybackslash}m{0.160\linewidth}}{\textbf{비고}} \\
\midrule
\endhead
...
\texttt{destina\allowbreak{}tion\allowbreak{}Id} & 없음 & \texttt{city\_\allowbreak{}id}, \texttt{destination\_\allowbreak{}id} metadata, Neptune \texttt{City.\allowbreak{}city\_\allowbreak{}id} & 추천 후보와 지도 마커는 검색·수집 보조 저장소 기준 \\
...
TTL & 로그성 테이블은 \texttt{expires\_\allowbreak{}at}을 필수 속성으로 둔다. 권고·잠정 보존 기간은 5.1절을 따른다(법무·보안 검토로 확정). \\
...
\DocNeedspace{24\baselineskip}
\subsection*{5.1 보존 기간 및 TTL (권고·잠정)}
\addcontentsline{toc}{subsection}{5.1 보존 기간 및 TTL (권고·잠정)}


\DocNeedspace{3\baselineskip}
아래 기간은 운영 관행 기준 \textbf{권고·잠정값(신뢰도 중)}이며, 법무·보안·개인정보 검토로 확정한다.

\DocNeedspace{3\baselineskip}
\texttt{expires\_\allowbreak{}at}은 레코드 생성 시각 + 보존 기간으로 계산해 저장한다.\newline 세부 근거는

\DocNeedspace{3\baselineskip}
\texttt{database\_\allowbreak{}design\_\allowbreak{}retention\_\allowbreak{}neptune\_\allowbreak{}update.\allowbreak{}md}를 따른다.


\DocNeedspace{24\baselineskip}
\begin{small}
\begin{longtable}{@{}>{\RaggedRight\arraybackslash}m{0.382\linewidth}>{\RaggedRight\arraybackslash}m{0.356\linewidth}>{\RaggedRight\arraybackslash}m{0.182\linewidth}@{}}
\toprule
\multicolumn{1}{>{\centering\arraybackslash}m{0.382\linewidth}}{\textbf{대상}} & \multicolumn{1}{>{\centering\arraybackslash}m{0.356\linewidth}}{\textbf{권고·잠정 보존 기간(TTL)}} & \multicolumn{1}{>{\centering\arraybackslash}m{0.182\linewidth}}{\textbf{비고}} \\
\midrule
\endhead
...
\texttt{lovv\_\allowbreak{}festival\_\allowbreak{}verify\_\allowbreak{}cache} & \texttt{confirmed} 30일 / \texttt{tentative} 7일 / \texttt{unknown·outdated} 1일 & 기존 확정값 \\
...
사용자 계정 & 탈퇴 후 30일 유예 $\rightarrow$ 익명화 & 분쟁/오삭제 대비, 법무 확인 필요 \\
...
Production 고도화 & 개인화 랭킹 모델 재학습, A/B 테스트 이벤트, S3 vector index 재색인 자동화, AWS Neptune 그래프 관계 탐색(비용 검증 후 도입, 그 전까지 \texttt{neptune\_\allowbreak{}alternative.\allowbreak{}md} 대체 아키텍처) \\
\bottomrule
\end{longtable}
\end{small}
...
\item [ ] S3 vector index 재색인 배치가 DynamoDB 정규화 문서와 S3 Raw 원본 기준으로 재생성되는가?
\item [ ] AWS Neptune 그래프 적재 배치가 공용 콘텐츠 관계만 노드·edge로 생성하는가?
...
\item DynamoDB \texttt{lovv\_\allowbreak{}user\_\allowbreak{}event\_\allowbreak{}logs}, \texttt{lovv\_\allowbreak{}agent\_\allowbreak{}runs}, \texttt{lovv\_\allowbreak{}async\_\allowbreak{}jobs}, \texttt{lovv\_\allowbreak{}api\_\allowbreak{}logs}의 TTL 기간을 확정한다. (5.1절에 권고·잠정값 반영 완료, 법무·보안 검토로 최종 확정 예정)
\item API 명세의 \texttt{/\allowbreak{}me/\allowbreak{}itineraries}, \texttt{/\allowbreak{}me/\allowbreak{}itineraries/\allowbreak{}\{\allowbreak{}itinerary\allowbreak{}Id\allowbreak{}\}}, \texttt{/\allowbreak{}me/\allowbreak{}itineraries/\allowbreak{}\{\allowbreak{}itinerary\allowbreak{}Id\allowbreak{}\}/\allowbreak{}reactions} 응답 필드와 테이블 컬럼을 1:1로 매핑한다.
...
v0.3 & {\footnotesize 2026-\allowbreak{}06-\allowbreak{}07} & 로브 기획팀 & 별도 벡터 DB 전제를 S3 vector 기능 기반 RAG 인덱스로 정리하고, VisitorStatistics와 S3 Raw/Lambda/DynamoDB 정규화 흐름 반영 \\
v0.4 & {\footnotesize 2026-\allowbreak{}06-\allowbreak{}08} & 로브 기획팀 & AWS Neptune을 추천 관계 탐색용 그래프 인덱스로 반영 \\
v0.5 & {\footnotesize 2026-\allowbreak{}06-\allowbreak{}08} & 로브 기획팀 & 보존 기간·TTL 권고·잠정값(5.1절) 추가, Neptune 비용 주의·대체 아키텍처(\texttt{neptune\_\allowbreak{}alternative.\allowbreak{}md}) 반영, 운영 대시보드(CloudWatch·Budgets)를 Production 운영 단계에 추가 \\
\bottomrule
\end{longtable}
\end{footnotesize}
