...
PDF 가독성을 높이는 작업은 기본적으로 원본 Markdown을 수정하지 않는다. 사용자가 "가독성", "표가 안 읽힘", "페이지 나눔", "목차 양식", "현재 PDF에 바로 적용"처럼 출력물 품질을 요청하면 먼저 현재 PDF의 `.tex`를 직접 조정하고 PDF를 다시 빌드한다. 같은 보정이 여러 문서에 반복될 때만 `scripts/markdown_to_tex.py` 또는 이 파일의 공통 규칙으로 승격한다.
...
- 테스트 계획서 원본: `docs/10_test_plan/10_test_plan.md`
- 테스트 계획서 TeX: `pdf/test_plan.tex`
- 테스트 계획서 PDF: `pdf/test_plan.pdf`
- 현재 AWS 시스템 구성도 원본: `docs/11_deployment_ops/supplemental/current_aws_system_architecture.md`
- 현재 AWS 시스템 구성도 TeX: `pdf/current_aws_system_architecture.tex`
- 현재 AWS 시스템 구성도 PDF: `pdf/current_aws_system_architecture.pdf`
- 시스템 아키텍처 및 구성도 원본: `docs/13_system_architecture/13_system_architecture.md`
- 시스템 아키텍처 및 구성도 TeX: `pdf/13_system_architecture.tex`
- 시스템 아키텍처 및 구성도 PDF: `pdf/13_system_architecture.pdf`
...
- 페이지 넘김, 표 폭, 코드 블록 줄바꿈, 표지 밀도, 로고 위치, 폰트, 색상 적용, PDF-only 풀어쓰기처럼 출력 가독성만 바뀌면 원본 Markdown을 수정하지 않고 `pdf/*.tex`와 `pdf/*.pdf`에 바로 적용한다.
- 현재 문서에만 필요한 단발성 가독성 보정은 `pdf/*.tex`에 직접 반영하고, 사용자가 원본 동기화를 요청하지 않는 한 `docs/`와 `pages/`는 그대로 둔다.
- PDF-only 변경 후에는 `git diff --name-only -- docs pages`가 비어 있는지 확인해 원본/HTML을 의도치 않게 건드리지 않았음을 검증한다.

프로젝트 기획서 기본 생성 명령은 다음을 사용한다.

```powershell
python scripts\markdown_to_tex.py docs\00_project_plan\00_project_plan.md pdf\project_plan.tex --title "프로젝트 기획서" --author "이창우, 전종혁, 조동휘, 최수아" --mentor "멘토 최민수" --team "조라에몽의 만능 도구들" --service-label "로브 서비스" --section-pagebreak --ci-images "../assets/images/SK-Networks-logo.png" "../assets/images/en-core-logo.png" "../assets/images/playdata-logo.png"
...
- 표는 본문 좌우 폭과 맞되 셀 내부가 빽빽하지 않도록 생성한다. `scripts/markdown_to_tex.py`의 `table_target_width`는 열 수와 `\tabcolsep=3pt`를 고려해 약 0.915~0.972 `\linewidth` 범위를 사용하고, 표별 행간은 1.42~1.58 범위에서 확보한다. longtable은 `\LTleft=\fill`, `\LTright=\fill`로 가운데 정렬한다.
- 목차는 기본적으로 2단계 깊이까지만 표시한다. 단, `requirements_definition`처럼 본문 표를 요약하고 상세 부록을 붙이는 문서는 독자가 세부 표 위치를 찾을 수 있도록 `tocdepth=3` 예외를 둘 수 있다.
- 큰 섹션은 `--section-pagebreak` 기준으로 새 페이지에서 시작한다.
- 프로젝트 기획서 `7.2 추천 기능`은 표 가독성을 위해 새 페이지에서 시작한다.
- 프로젝트 기획서 `7.4.2 B2B: 성과형 제휴 트랙`은 비즈니스 모델 하위 구분 가독성을 위해 새 페이지에서 시작한다.
...
- 목차는 본문 시작 전에 배치하고, 기본적으로 2단계 제목까지만 표시해 과도하게 길어지지 않게 한다. 긴 표를 요약/부록 구조로 나눈 문서는 3단계 목차를 제한적으로 사용한다.
...
- PDF 가독성 요청은 원본 Markdown 정리 작업으로 확대하지 않는다. 현재 PDF에서 독자가 보는 문제를 먼저 고치고, 필요할 때만 변환 규칙으로 일반화한다.
- `requirements_definition`의 `4. 기능 요구사항` 소개와 `4.1 공통 사용자 인터페이스` 사이에는 강제 페이지 넘김을 두지 않는다. 4장 소개가 짧으면 4.1 제목과 표가 같은 페이지에서 이어지도록 유지한다.
- `requirements_definition`의 `4. 기능 요구사항` longtable이 다음 페이지로 넘어가면 반복 머리글에 `4.x ... (계속)`을 표시한다. 4장 표는 행 수가 많아 다음 페이지에서 섹션 맥락이 사라지기 쉬우므로 `\DocTableContinue{열수}{섹션명}` 형태의 PDF 전용 머리글을 유지한다. 4.6 내부 표의 첫 제목도 `4.6 서비스별 세부 기능 - ...` 형식으로 유지해 페이지가 분리되어도 표의 소속을 알 수 있게 한다.
- `requirements_definition`에서 표 셀 설명이 길어지면 본문 표는 `핵심 요구`와 `상세 위치` 중심의 요약 표로 유지하고, 원문 수준의 상세 설명은 부록으로 분리한다. 기능 요구사항은 `부록 A. 기능 요구사항 상세`, 비기능·데이터 요구사항은 `부록 B. 비기능 및 데이터 요구사항 상세`에 둔다. 부록을 추가해도 `변경 이력`은 문서 마지막에 둔다.
- `requirements_definition`의 4.6처럼 하나의 절 안에 독립 검토 단위가 여러 개 있으면 `tocdepth=3`과 `\addcontentsline{toc}{subsubsection}{...}`를 사용해 3단계 목차에 노출한다. 단, 모든 표를 무조건 3단계 목차에 넣지 말고 독자가 찾기 어려운 긴 표 묶음에만 적용한다.
...
- `2.1 사용자 문제`
- `2.2 시장·운영 문제`
- `13. 후속 문서 및 분리 문서`: 대표 기획서 PDF에서는 내부 관리용 보조 Markdown 경로, 보조 Markdown 표, `supplemental/*.md` 참조를 노출하지 않는다.
- `requirements_definition`의 `2. 이해관계자`: Markdown 원본의 단순 문단 나열은 PDF에서 `구분 / 대표 이해관계자 / 주요 역할과 범위` 요약 표로 재구성할 수 있다. 이는 내용 변경이 아니라 PDF 스캔성 보정으로 취급한다.
- `requirements_definition`의 `3.1 서비스 소개`: 긴 설명 문단은 PDF에서 `구분 / 핵심 내용` 2열 요약 표로 재구성할 수 있다. 서비스 정의, 추천 방식, PoC/P1 범위, 일본 트랙, 서비스명 의미, 연동 데이터, 확장·제외 범위는 서로 분리해 스캔성을 높인다.
- `requirements_definition`의 `4.6 서비스별 세부 기능`: 요구사항 명세서 PDF에서는 내부 관리용 보조 Markdown 경로, 보조 Markdown 표, `supplemental/*.md` 참조를 노출하지 않는다. P5 항목은 상세 기능 요구사항 표에 넣지 않고 `후보 / 검토 조건` 2열 표로만 유지한다.
- `requirements_definition`의 `5. API 연동 요구사항`: Markdown 원본의 아이콘·카드형 플랫폼 소개가 PDF에서 평문 목록으로 풀리면 `플랫폼 / 주요 역할 / 연동 상태 / 적용 범위` 개요 표로 재구성한다. Yahoo Japan의 SDK 지원 중단·CORS 주의 문단은 상세 표의 `주의`, `CORS 제약` 행으로 흡수해 제목과 표가 분리되지 않게 한다.
- `requirements_definition`의 `8.2 가정사항`: 긴 bullet 목록은 PDF에서 `가정 항목 / 기준 / 적용 범위` 3열 표로 재구성한다. PoC 전제, 데이터 정책, 제외 범위, P5 승격 조건이 한 페이지에서 문단 덩어리처럼 보이지 않게 분리한다.
...
- `6.1 우선순위`: `기능 범위`를 가장 넓게 두고, `우선순위`, `PoC`, `Production`, `상태`는 가운데 정렬한다.
- `7.2 추천 기능`: `기능`은 좁고 가운데 정렬, `설명`은 넓게 배치한다.
- `7.4.3 B2G`: `구분`과 `성과 지표`는 좁고 가운데 정렬, `운영 기준`은 넓게 배치한다.
- `8.1 필요 데이터`: `구분`은 좁고 가운데 정렬, `데이터 범위`와 `활용 기준`은 설명 폭을 확보한다.
- `8.2 외부 연동`: `구분` 또는 `운영 항목`은 좁고 가운데 정렬, `연동 대상`과 `운영 기준` 또는 `폴백·제한`은 설명 폭을 확보한다.
- `13. 후속 문서 및 분리 문서`: 첫 열은 좁고 가운데 정렬, 설명 열은 넓게 배치한다. 내부 관리용 보조 Markdown 표는 PDF에서 제외한다.
...
- `requirements_definition`의 `4.6 우선순위 기능 요구사항`: `요구사항 내용` 열을 가장 넓게 두고, `비고` 열은 보조 설명이 읽히는 범위까지만 줄인다. 긴 FR ID는 `\allowbreak{}`로 줄바꿈 가능하게 유지한다.
- `requirements_definition`의 `4.6 향후 검토 후보`: 후보명 열은 좁게, `검토 조건` 열은 넓게 배치한다. 근거 파일명이나 보조 문서 경로 열은 만들지 않는다.
- `requirements_definition`의 `2.1 역할 모델`: `주요 책임` 열을 가장 넓게 두고, Role ID와 인증 열은 줄바꿈 가능한 범위로만 확보한다.
- `requirements_definition`의 `2.2 권한 매트릭스`: 6열 표이므로 `\tabcolsep=2pt`를 사용하고, `기능/데이터`, `관광 운영자`, `데이터 제공 기관`처럼 줄바꿈이 잦은 열에 폭을 우선 배정한다.
- `requirements_definition`의 `2.3 데이터 접근 범위`: `데이터 범위` 열을 너무 좁히지 말고, 긴 설명은 `포함 데이터`와 `허용 역할` 열에서 자연스럽게 줄바꿈되도록 유지한다.
- `requirements_definition`의 `3.1 서비스 소개`: `구분` 열은 좁고 가운데 정렬, `핵심 내용` 열은 넓게 배치한다. 일본 트랙, 연동 데이터, 확장·제외 범위처럼 보류·제외 조건이 섞인 행은 한 행 안에서 읽히도록 설명 폭을 우선 확보한다.
- `requirements_definition`의 `3.2 핵심 가치 제안`: 긴 설명 열 하나에 내용을 몰아넣지 말고 `가치 항목 / 핵심 가치 / 적용 기준` 3열로 나눈다. 첫 열은 짧은 라벨만 두고, PoC/P1·P5 같은 범위 조건은 `적용 기준` 열에서 분리해 읽히게 한다.
- `requirements_definition`의 `4.8 추천 파이프라인 명세`: `입력`과 `판단 기준`을 좁은 별도 열로 나누면 긴 단계가 빽빽해지므로 PDF에서는 `단계 / 입력·판단 기준 / 출력` 3열로 정리한다. `단계` 열은 긴 단계명이 과하게 꺾이지 않도록 약 `0.24\linewidth` 수준으로 확보하고, `입력·판단 기준` 열 안에서 라벨과 줄바꿈으로 두 정보를 분리한다.
- `requirements_definition`의 `5. API 연동 요구사항`: 첫 플랫폼 소개는 4열 개요 표로 정리하되 `주요 역할` 열을 가장 넓게 둔다. 5.1~5.4 상세 표는 2열을 유지하고 `항목` 열을 좁게, `내용` 열을 넓게 배치한다. 상세 표가 다음 페이지로 넘어가면 `\DocTableContinue{2}{5.x ...}` 반복 머리글을 둔다.
- `requirements_definition`의 `5.5 API 연동 현황 요약`: 5.4 Yahoo Japan 표 뒤 여백이 충분하면 다음 페이지로 밀지 말고 같은 페이지 하단에 붙인다. 5.5 직전 `\DocNeedspace`는 표 전체 고정이 아니라 제목 단독 고아 방지 수준으로만 둔다.
- `requirements_definition`의 `4.2`~`4.5`, `4.6.2`, `4.9`, `6.2`, `7.1`, `7.2`처럼 본문 표 행이 많거나 셀 설명이 긴 표는 본문에서 요약 표로 줄이고 상세 설명을 부록 A/B로 분리한다. 본문 행의 마지막 열은 `부록 A.x` 또는 `부록 B.x` 형식으로 상세 위치를 명시한다.
- `requirements_definition`의 `8.2 가정사항`: 첫 열은 짧은 라벨만 두고, `기준` 열에 실제 전제를 넓게 배치한다. `적용 범위` 열에는 PoC, 데이터 설계, 운영 권한, P5 후보 관리처럼 검토자가 빠르게 분류할 수 있는 기준을 둔다.
- `requirements_definition`의 `9.5.2 스프린트별 개발 기한`: 제목만 페이지 하단에 남지 않도록 제목과 스프린트 표가 같은 페이지에서 시작하게 한다.
...
```powershell
python scripts\markdown_to_tex.py docs\03_data_collect_plan\03_data_collect_plan.md pdf\data_collect_plan.tex --title "데이터 수집 계획서" --author "이창우, 전종혁, 조동휘, 최수아" --mentor "멘토 최민수" --team "조라에몽의 만능 도구들" --service-label "로브 서비스 데이터 수집 계획" --section-pagebreak --body-pagebreak-before "2.3 City 수집 항목" "2.4 Attraction 수집 항목" "2.6 데이터 출처" "2.6.2 일본 데이터 출처" --ci-images "../assets/images/SK-Networks-logo.png" "../assets/images/en-core-logo.png" "../assets/images/playdata-logo.png"
Push-Location pdf
...
테스트 계획서 기본 생성 명령은 다음을 사용한다.

```powershell
python scripts\markdown_to_tex.py docs\10_test_plan\10_test_plan.md pdf\test_plan.tex --title "테스트 계획서" --author "이창우, 전종혁, 조동휘, 최수아" --mentor "멘토 최민수" --team "조라에몽의 만능 도구들" --service-label "로브 서비스 테스트 계획" --section-pagebreak --ci-images "../assets/images/SK-Networks-logo.png" "../assets/images/en-core-logo.png" "../assets/images/playdata-logo.png"
Push-Location pdf
xelatex -interaction=nonstopmode -halt-on-error test_plan.tex
xelatex -interaction=nonstopmode -halt-on-error test_plan.tex
Select-String -Path test_plan.log -Pattern 'Overfull \\hbox'
Pop-Location
```
