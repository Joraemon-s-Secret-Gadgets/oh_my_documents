...
요구사항 정의서의 이해관계자 역할 모델은 후속 문서 작성 시 공통 기준으로 사용합니다.

- 사용자 시나리오에는 `R-USER`, `R-LOCAL-OPERATOR`, `R-ADMIN`, `R-DATA-PROVIDER`별 대표 흐름을 분리합니다.
- 화면 흐름 / 정보 구조에는 공개 화면과 운영자·관리자 화면의 접근 경로 및 권한 없는 상태를 포함합니다.
- 데이터베이스 설계에는 역할, 권한, 기관 소속, 담당 지역, 승인 이력, 감사 로그 후보를 검토합니다.
- API 명세서에는 인증 필요 엔드포인트, 역할별 인가 조건, 401/403/키 만료 응답을 정의합니다.
- 테스트 계획서에는 역할별 접근 제어, 담당 지역 제한, 제안·승인 분리, API Key 장애 폴백 검증을 포함합니다.
