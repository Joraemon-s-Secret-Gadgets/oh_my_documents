...
> 기준 문서: `docs/06_agent_spec/06_agent_spec.md`

# 1. 문서 목적

본 문서는 로브(Lovv) 서비스에서 구현할 Agent의 종류와 책임 범위를 정리한다.
상세 입출력, 상태 전이, 도구 사용 규칙은 `06_agent_spec.md`를 기준으로 하며, 이 문서는 개발자가 구현 대상을 빠르게 파악하기 위한 목록형 문서로 사용한다.

# 2. 구축할 Agent 개요

로브 Agent는 하나의 거대한 Agent가 모든 작업을 처리하는 구조가 아니라, 의도 정리 Agent와 중앙 제어 Agent가 여러 전문 Agent를 호출하는 멀티 Agent 파이프라인으로 구성한다.

멀티턴 대화에서는 `Intent_Agent`가 사용자 의도와 선호를 먼저 정리하고, `Supervisor_Router`에는 라우팅에 필요한 최소 컨텍스트만 전달한다. 본 문서의 `Condition_Parser_Agent`는 초기 구현 단계에서는 `Intent_Agent` 내부 기능으로 구현할 수 있으며, 이후 필요하면 별도 노드로 분리한다.

| 구분 | Agent | 핵심 책임 |
| --- | --- | --- |
| 의도 정리 | `Intent_Agent` | 멀티턴 대화 정리, 불필요한 컨텍스트 제거, handoff payload 생성 |
| 중앙 제어 | `Supervisor_Router` | 전체 실행 흐름 제어, 상태 점검, 다음 Agent 호출 결정 |
| 입력 해석 | `Condition_Parser_Agent` | 사용자 자연어와 UI 조건을 구조화된 여행 조건으로 변환 |
...
| 주요 출력 | `extracted_inputs`, `user_preferences`, `fulfilled_matrix`, `excluded_themes` |
...
| 중요 규칙 | 각 Agent 실행 후 제어권은 반드시 Supervisor로 돌아온다 |

## 3.3 Condition_Parser_Agent

사용자의 자연어 요청과 UI 입력값을 추천 조건 객체로 변환하는 Agent다.
필수 조건이 부족하면 바로 추천을 생성하지 않고 추가 질문이 필요한 상태로 넘긴다.

| 항목 | 내용 |
| --- | --- |
| 구현 목적 | 사용자 요청을 시스템이 처리 가능한 조건으로 구조화 |
| 주요 입력 | 자연어 요청, 진입 경로, 국가, 월, 일정 유형 |
| 주요 출력 | `country`, `travelMonth`, `tripType`, `theme`, `entryType` |
| 폴백 | 필수 조건 누락 시 추가 질문 생성 |
| 구현 위치 | 초기 구현에서는 `Intent_Agent` 내부 기능으로 포함 가능 |

## 3.4 Polymorphic_Retriever_Agent
...
사용자 조건, 계절 적합도, 혼잡도, 기상 경향, 좋아요/싫어요 이력을 점수에 반영한다.
...
| 중요 규칙 | 검증 실패 시 최종 응답을 바로 사용자에게 반환하지 않고 `Supervisor_Router`로 되돌려 재탐색, 재작성, 폴백 확정 중 하나를 선택하게 한다 |

## 3.10 Backend_Serving / SAM
...
| 우선순위 | Agent | 이유 |
| --- | --- | --- |
| 1 | `Intent_Agent` / `Condition_Parser_Agent` | 사용자 의도, 입력 조건, Supervisor 전달 payload 구조를 결정 |
...
| `Map Link Builder` | Tool 유지 | 지도 딥링크 생성은 단순 변환 작업 |
| `Stay Link Builder` | Tool 유지 | 숙소 검색 링크 생성은 단순 변환 작업 |
