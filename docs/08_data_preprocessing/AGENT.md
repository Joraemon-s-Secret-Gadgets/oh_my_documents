...
- `data_preprocessing_plan.md` is the integrated representative document for this folder.
- `sections/*.md` is the editing source for `data_preprocessing_plan.md`.
- When changing representative document body content, edit the matching `sections/*.md` file first, then regenerate or update `data_preprocessing_plan.md` in the same change.
...
supplemental/preprocessing_budget_estimate.md       전처리 PoC 예산 산정
supplemental/kr_preprocessing_detail_design.md      한국(KR) ELT 전처리 상세 설계 (설계 의도 기준)
...
- 한국 실수집 기준: `../03_data_collect_plan/supplemental/korea_data_acquisition_plan_updated.md`
- 적재 설계 기준: `../04_database_design/supplemental/nosql_schema_design.md`
...
- 수량·필드는 실수집 기준(`../03_data_collect_plan/supplemental/korea_data_acquisition_plan_updated.md`)과 일치시키고, 불일치는 신뢰도와 함께 명시한다.
