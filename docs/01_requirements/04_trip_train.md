...
<html lang="ko">
  <head>
    ...
    <style>
      ...
      * {
      ...
      /* Pass Savings Calculator Widget */
      .calculator-card {
        background: var(--bg-card);
        border: 1px solid var(--border-color);
        border-radius: 24px;
      ...
      /* Platform Synergy / BM Cards Grid */
      .biz-value-grid {
        display: grid;
        grid-template-columns: repeat(3, 1fr);
        gap: 1.5rem;
  ...
  <body>
    ...
    <script>
      ...
      // Feature Showcase switcher logic
      ...
      const chatScenarios = [
        [
          {
            sender: "user",
            text: "부모님을 모시고 온천도 하면서 조용히 쉴 수 있는 기차 패키지 여행 코스를 알려줘. 갈아타는 건 최대한 없었으면 좋겠어.",
          },
          {
            sender: "assistant",
            text: "안녕하세요! 부모님과 함께하는 편안한 힐링 온천 여행이시군요. 복잡한 도심을 피해 기차로 한 번에 갈 수 있는 명소를 탐색하겠습니다. 잠시만 기다려주세요...",
          },
        ],
        [
          {
          ...
          {
            sender: "assistant",
            text: "규슈 소도시 유후인의 료칸 온천 코스를 제안드립니다. 하카타역에서 출발하는 전용 특급 열차 <strong>유후인의 모리</strong>를 타시면 환승 없이 유후인역에 직통으로 도달할 수 있습니다. 개별 구간 예매 대비 교통 비용을 최대 35% 이상 줄일 수 있는 JR 규슈 레일패스를 연계하는 동선이 가장 효과적입니다.",
          },
        ],
        [
          {
          ...
          {
            sender: "assistant",
            text: "⚙️ <strong>[검증 완료]</strong> 7월 첫째 주 주말 기준으로 유후인의 모리 특급 열차의 잔여 지정석 현황 및 해당 소도시의 대표 온천 료칸 패키지 3곳의 예약이 가능함을 파악했습니다. 소도시 인구 혼잡도 시뮬레이션상 해당 주말 유후인은 저혼잡(LOW) 등급으로, 여유로운 휴식이 보장됩니다.",
          },
        ],
        [
          {
          ...
          {
            sender: "assistant",
            text: '🎟️ <strong>추천 상품</strong>: [규슈 유후인 료칸 & 온천 3일 자유 패키지 - 특급 열차권 포함]<br>👉 <a href="#" style="color:var(--accent-blue); text-decoration:underline;">실시간 여행사 에이전시 공식 예약 및 결제하기</a>',
          },
        ],
      ];

      function setChatScenario(stepIdx) {
        // Toggle timeline node active classes
        ...
        // Populate chat messages with offset delays
      ...
      const calcData = [
        {
          rawCost: 384000,
          passName: "JR 규슈 레일패스 (5일권)",
          passCost: 240000,
          saving: 144000,
        },
        {
          rawCost: 462000,
          passName: "JR 호쿠리쿠 아치 패스 (7일권)",
          passCost: 280000,
          saving: 182000,
        },
        {
          rawCost: 290000,
          passName: "코레일 내일로 패스 (7일권 선택형)",
          passCost: 110000,
          saving: 180000,
        },
      ];

      let currentSelectedRouteIdx = 0;

      function selectCalcRoute(routeIdx) {
        const items = document.querySelectorAll(".calc-route-item");
        items.forEach((item, idx) => {
          if (idx === routeIdx) item.classList.add("selected");
          else item.classList.remove("selected");
        });
        currentSelectedRouteIdx = routeIdx;
      }

      function runCalculation() {
        const data = calcData[currentSelectedRouteIdx];

        // Update values with visual counts (simulated)
        document.getElementById("lbl-raw-cost").innerText =
          `KRW ${data.rawCost.toLocaleString()}`;
        document.getElementById("lbl-pass-name").innerText = data.passName;
        document.getElementById("lbl-pass-cost").innerText =
          `KRW ${data.passCost.toLocaleString()}`;

        const savingEl = document.getElementById("lbl-saving-val");
        savingEl.innerText = `KRW ${data.saving.toLocaleString()}`;
        savingEl.style.transform = "scale(1.15)";
        setTimeout(() => {
          savingEl.style.transform = "scale(1)";
        }, 200);
      }

      // Initial setups on window load
      window.onload = () => {
        setChatScenario(0);
        runCalculation();
      };
