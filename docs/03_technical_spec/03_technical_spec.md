...
| Recommendation Module | Agent 실행, Supervisor Router를 통한 상태 제어 및 순환 루프 제어, 후보 선정, 일정 생성 |
...
| 실시간 API 교차 검증 | 당해 연도 정확한 개최 일정/기간 등 실시간 최신화가 필수적인 특정 항목에 한해, Polymorphic_Retriever_Node가 런타임에 외부 API를 실시간 호출해 데이터를 교차 검증하고 정적 필드를 오버라이드(Override)하는 하이브리드 데이터 연동 |
