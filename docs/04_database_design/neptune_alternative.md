# 로브 (Lovv) Neptune 대체 설계 문서

> 문서 버전: v0.1
> 문서 상태: 검토 중 (Review)
> 대표 문서: `04_database_design.md` (v0.4) 보조 문서
> 관련 초안: `database_design_retention_neptune_update.md`

# 1. 배경

대표 문서는 다단계 관계 탐색용 그래프 인덱스로 AWS Neptune을 정의한다(1.2·3.5·4.6절).
...
대체하는 설계를 정의한다. 그래프 규모·질의 복잡도가 커지면 Neptune 도입(고도화 단계)으로 승격한다.
...
대표 문서 3.5절(그래프 논리 모델)·4.6절(조회 패턴)의 요소를 1:1로 대응한다.
...
| 2-hop 확장 + 그래프 재랭킹 | 다단계 후보 확장·재정렬 | 배치 사전계산 머티리얼라이즈 또는 요청 시 인메모리 그래프 탐색 | DynamoDB / Lambda 인메모리 |
...
- 위 요건이 검증되면 Neptune(고도화 단계, 대표 문서 6장) 도입을 결정한다.

# 8. 대표 문서 반영 위치

- 1.2절 설계 기준 표 `Graph DB / AWS Neptune` 비고: "예산 제약상 보류, 본 대체 문서 적용, 고도화 단계 도입".
- 1.3절 저장소 책임 표 Neptune 행: "상시 비용 발생, 대체 아키텍처(neptune_alternative.md) 우선".
- 3.5·4.6절: "PoC/Prod 1차 대체 구현은 neptune_alternative.md" 각주.
