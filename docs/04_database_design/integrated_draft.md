...
> 최신 반영 메모(2026-06-12): 이 초안의 Graph DB/Neptune 표현은 비용 검토 전 가정이다. 현재 대표 문서는 그래프DB 직접 도입 대신 Lambda 기반 관계 탐색 보조 기능을 구현하고, Neptune은 고도화 승격 옵션으로만 둔다.
...
| 관계 탐색 보조 | Lambda + DynamoDB 인접 리스트 | 도시·축제·테마·장소 관계 탐색은 S3 vector 의미 검색과 분리하되, 그래프DB 직접 도입 대신 Lambda 보조 기능으로 관리한다. |
...
서비스 아키텍처는 MySQL 8 LTS, S3 vector 기반 RAG Index, Lambda 관계 탐색 보조, DynamoDB Logs의 4개 책임으로 나눈다.
MySQL은 정합성과 관계가 필요한 원장 데이터, S3 vector 기반 RAG Index는 의미 검색용 복제 인덱스, Lambda 관계 탐색 보조는 도시·축제·테마 관계 후보 확장, DynamoDB는 대량·비정형·TTL 로그를 담당한다.
...
| Lambda 관계 탐색 보조 | 추천 후보 확장과 관계 기반 재랭킹을 위한 보조 기능 | 도시-테마, 도시-축제, 도시-인접 도시, 장소 후보 관계 |
...
| 목적지, 축제, 장소, 체험, 출처 | MySQL | 검색용 청크와 임베딩은 S3 vector index, 관계 탐색은 Lambda 관계 탐색 보조 |
...
![Lovv 통합 데이터베이스 ERD](../../assets/images/mermaid/04-database-design-integrated-draft-01.png)
...
S3 vector index는 MySQL 원장 또는 수집 정규화 데이터를 검색용으로 복제한 의미 검색 인덱스다.
따라서 S3 vector index의 데이터가 원본이 아니며, 재색인 또는 장애 복구 시 MySQL 원장, DynamoDB 정규화 문서, S3 Raw 원본을 기준으로 다시 생성할 수 있어야 한다.

| 인덱스 대상 | 원본 데이터 | S3 vector 저장 내용 |
| --- | --- | --- |
...
### Lambda Relationship Helper

Lambda 관계 탐색 보조 기능은 S3 vector 검색 결과를 도시·축제·테마·장소 관계로 확장하는 파생 기능이다.
관계 탐색 캐시는 원본 저장소가 아니며, MySQL 원장, DynamoDB 정규화 문서, S3 Raw 원본을 기준으로 재생성할 수 있어야 한다.

| 그래프 요소 | 라벨/관계 | 용도 |
| --- | --- | --- |
| 도시 | `City` | 추천 후보의 중심 노드 |
| 축제 | `Festival` | 여행 연도와 축제 개최 상태 연결 |
| 테마 | `Theme` | 필수·선호 테마 충족 관계 탐색 |
| 장소 | `Place` | 일정 항목 후보 확장 |
| 관계 | `HAS_THEME`, `HOSTS_FESTIVAL`, `NEAR_CITY`, `HAS_PLACE`, `SEASONAL_FIT` | 그래프 기반 후보 확장과 재랭킹 |

관계 탐색 입력과 캐시에는 사용자 개인정보, 대화 전문, 비공개 운영 메모를 저장하지 않는다.
...
7. AWS Neptune의 노드 라벨, edge 라벨, 관계 가중치, 재적재 배치 기준을 결정한다.
8. DynamoDB 로그 그룹별 파티션 키, 정렬 키, TTL 기준을 확정한다.
9. Raw 수집 데이터 저장 위치(S3 또는 파일 저장소), 보존 기간, 해시 정책을 확정한다.
10. `chat_messages`를 실제 구현할 경우 TTL 삭제 배치와 개인정보 마스킹 정책을 먼저 정의한다.

# 11. 변경 이력

| 버전 | 날짜 | 작성자 | 변경 내용 |
| --- | --- | --- | --- |
| v0.3 | 2026-06-08 | 로브 기획팀 | 초안 당시 Neptune 기반 관계 탐색 파생 인덱스 검토 내용을 반영 |
