...
> 보조 문서: `database_design_retention_neptune_update.md`(보존기간·비용·대시보드), `neptune_alternative.md`(Neptune 대체 설계)
...
| Graph DB | AWS Neptune (고도화 단계) | 목적지, 축제, 테마, 인접 도시, 이동 관계처럼 다단계 관계 탐색이 필요한 추천 그래프를 관리한다. 단, Neptune은 상시 가동 비용(최소 1 NCU 약 월 $80+)이 높아 PoC/Prod 1차는 `neptune_alternative.md`의 대체 아키텍처를 적용하고, 고도화 단계에서 도입을 결정한다. |
...
| DynamoDB | 비정형 실행 상태, 이벤트 로그, TTL 로그, 수집 정규화 결과 | Agent 실행 상태, async job, 사용자 행동 이벤트, API 호출 로그, plan reaction clickstream, admin operation trace, City/Attraction/Festival/VisitorStatistics 정규화 문서 |
| S3 vector index | 의미 검색 인덱스 | 목적지·축제·관광지 문서 chunk, embedding, source reference, metadata filter |
| AWS Neptune | 그래프 관계 인덱스 (상시 비용 발생, 고도화 단계 도입) | 도시-테마, 도시-축제, 도시-인접 도시, 일정 항목-장소 후보 관계. PoC/Prod 1차는 `neptune_alternative.md` 대체 아키텍처 적용 |
...
| `lovv_agent_runs` | `pk` = `RUN#{agent_run_id}` | `sk` = `STATE#{created_at}` | `user_id_hash`, `session_id`, `recommendation_request_id`, `status`, `node_name`, `tool_name`, `next_node`, `fulfilled_matrix_summary`, `target_region`, `validation_retry_count`, `token_usage`, `error_code`, `payload_summary` | `expires_at` |
...
> 대체 설계는 `neptune_alternative.md`를 따른다. 본 3.5절/4.6절은 고도화 단계 Neptune 도입 시의 목표 모델이다.
...
| 관계 가중치 | `weight`, `confidence`, `source_id`, `updated_at`을 공통 edge 속성으로 둔다. |
...
`database_design_retention_neptune_update.md`를 따른다.
...
| `lovv_festival_verify_cache` | `confirmed` 30일 / `tentative` 7일 / `unknown·outdated` 1일 | 기존 확정값 |
...
| Production 고도화 | 개인화 랭킹 모델 재학습, A/B 테스트 이벤트, S3 vector index 재색인 자동화, AWS Neptune 그래프 관계 탐색(비용 검증 후 도입, 그 전까지 `neptune_alternative.md` 대체 아키텍처) |
...
5. AWS Neptune의 `City`, `Festival`, `Theme`, `Place` 노드와 관계 적재 배치 기준을 기술 명세와 맞춘다.
...
| v0.5 | 2026-06-08 | 로브 기획팀 | 보존 기간·TTL 권고·잠정값(5.1절) 추가, Neptune 비용 주의·대체 아키텍처(`neptune_alternative.md`) 반영, 운영 대시보드(CloudWatch·Budgets)를 Production 운영 단계에 추가 |
