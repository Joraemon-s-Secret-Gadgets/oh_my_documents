...
| Neptune graph record | TTL 없음, 원본 재생성 기준 운영 보존 | 원본 아님, 재생성 가능 | (해당 없음) |
...
| 옵션 | 내용 | 적용 단계 권고 |
| --- | --- | --- |
| A. 도입 보류(권고) | PoC/Production 1차에서는 Neptune 미도입. 그래프 관계를 DynamoDB 인접 리스트 또는 MySQL 관계 테이블로 대체하고, 2-hop 탐색은 사전 계산 인접 관계로 근사 | PoC ~ Prod 1차 |
| B. 지연 도입 | 그래프 다단계 탐색 효용이 검증된 고도화 단계에서만 Neptune 활성화 | Prod 고도화 |
| C. 최소 형상 + 모니터링 | 도입 시 Serverless 최소 NCU로 시작하고 사용량·비용 알람 설정 | 도입 시 공통 |

권고: 월 30만원 예산에서는 옵션 A(도입 보류)를 우선하고, 그래프 효용이 검증되면 옵션 B로 도입한다.
원본 1.2절 설계 기준 표의 `Graph DB` 행 비고에 비용 주의와 "예산 제약상 보류, 고도화 단계 도입(옵션 A→B)"을
명시하고, 6장 PoC/Production 전환 표의 Neptune 항목을 `Production 고도화` 단계로 유지·강조한다.
(원본 6장은 이미 Neptune을 "Production 고도화"에 두고 있어 옵션 B와 방향이 일치한다.)

### 3.4 대표 문서 반영 위치

- 1.2절 설계 기준 표 `Graph DB / AWS Neptune` 비고에 비용 주의 + 도입 단계 명시.
- 1.3절 저장소 책임 표 Neptune 행에 "상시 비용 발생, 고도화 단계 도입" 주석.
- 6장 전환 표 Neptune 항목에 비용 근거 각주.
...
- 3.5절 AWS Neptune 그래프 논리 모델에 "PoC/Prod 1차 대체 방안(3.5.3)" 주석 추가.
- 4.6절 Neptune 물리 설계 기준에 "도입 전 대체 구현" 각주.
