...
> 기준 PPTX: `artifacts/pptx/lovv_midterm_focus_current_s4_split.pptx`
> 동기화 기준: 2026-06-15 S4 카드 애니메이션 분리본.
...
Lovv 소개 → 오버투어리즘 정의·사례 → 한일 시장 근거 → 핵심 문제 카드(전) → Lovv 기능 카드(후) → 시스템 흐름 → 사용자 흐름 → Agent 구조 → Candidate Evidence → Planner → 데이터 → 데모 → 차별점 → 출처 → Q&A
...
## 슬라이드 4 — 핵심 문제와 Lovv 기능 (카드 애니메이션 전)
...
| 문제 카드 | 핵심 병목 | 하단 연결 |
| --- | --- | --- |
| 문제 01 | 유명지만 간다 | 대체 소도시 발견 |
| 문제 02 | 찾다가 지친다 | 조건에서 일정까지 |
| 문제 03 | 추천은 못 믿는다 | 근거가 보이는 추천 |

**발표자 노트**
- 사용자는 유명지를 좋아해서만 가는 것이 아니라 실패 비용을 피하려고 검증된 장소로 쏠린다.
- 먼저 세 가지 병목을 보여준 뒤, 다음 장에서 같은 카드 위로 Lovv 기능을 올려 연결한다.

---

## 슬라이드 5 — 핵심 문제와 Lovv 기능 (카드 애니메이션 후)
...
**발표자 노트**
- 전 장의 문제 카드를 유지한 상태에서 기능 카드가 올라온다.
