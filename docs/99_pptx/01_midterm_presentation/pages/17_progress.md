...
| 2주차 | 추천 근거·기준 정의 · 데이터 수집 · DB 설계 |
| 3주차 | 시스템 아키텍처·DB·멀티에이전트 재설계 · RAG·S3 Vector 설계 |
| 4주차 | PoC 구현 · 백엔드·인증 · 프론트 목업 배포 |
...
- **프레임** — 앞 장 협업 흐름으로 진행을 한눈에 추적 / 4주 / 문제 정의 → 중간 발표, 단계적으로 좁혀옴
- **스프린트 운영** — 1주 = 1 스프린트로 진행(Sprint 1~4), 단 1주차(Sprint 1)는 5/22~5/31 10일 킥오프
- **1주차** — 프로젝트 기획 · 요구사항 정의 · WBS · 협업 환경 구축
- **2주차** — 추천 근거 데이터 확정 / 한국 40개로 먼저 검증 후 일본 확장 / 권한 사용자·관리자로 단순화 / retriever 필터링→유사도→랭킹·festival DB 사전저장 방향 / 데이터 수집·DB 설계
...
- **마일스톤** — 외부 고정일 2개로 운영: M1 중간 발표(6/16) → M2 최종·Production(7/16)
