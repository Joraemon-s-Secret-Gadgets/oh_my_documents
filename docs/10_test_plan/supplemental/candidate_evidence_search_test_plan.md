...
> 기준 문서: `../../05_agent_spec/supplemental/candidate_evidence_agent.md`, `../../05_agent_spec/supplemental/candidate_evidence_baseline_comparison.md`
...
상세 차이는 `../../05_agent_spec/supplemental/candidate_evidence_baseline_comparison.md`를 따른다.
...
원본 실행 보고서의 요지는 본 문서와 `candidate_evidence_evaluation_results.md`에 반영한다. 외부 평가 하네스의 파일 경로는 정본 문서에 고정하지 않는다.
...
| 항목 | 설계 |
| --- | --- |
| 입력 | 평가 testset, Baseline raw output, Ours raw output |
| 실행 도구 | Blind review runner |
...
| 산출물 | 용도 |
| --- | --- |
| Evaluation report | 전략별 결과, latency, memory, retrieval metric |
| Baseline raw output | Baseline Candidate Package 원본 |
| Ours raw output | Ours Candidate Package와 quota audit 원본 |
| Blind result | 블라인드 judge 승패, 차원별 평균, disqualifier 집계 |
| Blind report | 블라인드 judge 요약 리포트 |
...
데이터와 embedding index가 바뀌면 이전 평가 산출물을 덮어쓰기 전에 실행 일자와 index version을 기록해야 한다. 동일 평가 로직이라도 외부 데이터 변경으로 결과가 달라질 수 있기 때문이다.
...
8. AWS index와 원본 데이터가 변경되면 동일 평가 로직의 결과도 달라질 수 있다.
...
| 버전 | 날짜 | 변경 내용 |
| --- | --- | --- |
| v1.2 | 2026-06-13 | 외부 평가 하네스 파일 경로와 실행 명령 직접 참조를 제거하고 절차·산출물 유형 중심으로 정리 |
| v1.1 | 2026-06-12 | LLM judge 기반 블라인드 pairwise 평가 설계와 게이트 반영 |
| v1.0 | 2026-06-12 | 도시·관광지 검색 비교·회귀·정성 평가 계획과 현재 합격 기준 정본화 |
