...
| `city_name_ko` | Wikipedia / 행정구역 데이터 / 수동 정규화 | 전부 수집 | 한국어 도시명 |
| `province` | 행정구역 데이터 | 전부 수집 | 광역시·도 |
| `district_type` | 행정구역 데이터 | 전부 수집 | 시·군·구 구분 |
| `location` | Wikipedia / TourAPI / 행정구역 데이터 | 전부 수집 | 위치 정보, 좌표 또는 행정 위치 |
| `latitude` | TourAPI / Wikidata / 지도 보조 | 전부 수집 | 대표 위도 |
| `longitude` | TourAPI / Wikidata / 지도 보조 | 전부 수집 | 대표 경도 |
| `description` | Wikipedia / 대한민국구석구석 기반 요약 | 전부 수집 | 도시 역사·문화·특징 설명 |
| `climate` | 기상청 API허브 / 기후통계 | 전부 수집 | 기후 개요 또는 월별 기후 메모 |
...
| 적용 방식 | City의 `climate`와 계절 추천 지표 산출에 사용한다. |
...
| 대상 | 자동 수집 소스 | 처리 방식 |
| --- | --- | --- |
| 도시 | Wikipedia, Wikidata, 행정구역 데이터, 기상청 | 도시명, 위치, 설명, 기후, 사이트 링크, 위도, 경도를 추출하고 City 테이블로 정규화한다. |
...
2. `2.4.1 대한민국 데이터 출처`에 TourAPI, 대한민국구석구석, 기상청, 행정구역 데이터의 역할을 표로 반영한다.
