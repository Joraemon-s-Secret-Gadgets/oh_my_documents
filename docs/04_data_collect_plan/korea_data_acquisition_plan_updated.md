...
> [변경] Wikipedia 페이지에서 geohack 지도 링크나 정적 좌표 정보(Wikidata 연동 포함)를 가져오지 못하는 경우가 존재하므로, OpenStreetMap의 **Nominatim Geocoding API**를 폴백(Fallback) 수집 수단으로 지정하였다. 대한민국 지자체 명칭(예: "대한민국 강원특별자치도 춘천시")을 지오코딩 쿼리로 사용하여 위경도 결측을 원천 방지한다. API 이용 제한 준수를 위해 요청 시 1초 대기 지연(Rate Limit Delay)을 강제 적용한다.
...
| 대상 | 자동 수집 소스 | 처리 방식 |
| --- | --- | --- |
| 도시 | Wikipedia, Wikidata, OpenStreetMap Nominatim API | 도시명, 위치, 설명, 기후, 사이트 링크, 위도, 경도를 추출하고 City 레코드로 정규화한다. (Wikipedia 좌표 결측 시 Nominatim 지오코딩 폴백 적용) |
