...
`anchor_fixed`와 `festival_selection` 모드에서는 Planner 진입 전에 도시가 이미 고정되어 있어야 한다. Planner는 이 anchor를 바꾸지 않는다.
...
| `candidate_evidence_package.mode` | `city_discovery`, `anchor_fixed`, `festival_selection` |
...
| `festival_selection` | Festival Verifier가 선택한 2026년 축제의 도시를 유지. 선택 축제는 가능한 경우 첫 일정 블록 또는 핵심 이벤트 블록으로 고려 |
...
API 응답은 현재 `timeOfDay`만 요구한다. 내부에서는 `flex`, `meal`, `rest`를 사용하되 Backend Serving에서 UI 계약에 맞게 표시명을 조정한다.
...
축제는 사용자가 `includeFestivals=true`를 선택했거나 자연어에서 축제 의도가 명확할 때만 일정에 반영한다.

| `date_status` | 처리 |
| --- | --- |
| `confirmed` | 일정 블록 직접 배치 가능 |
| `tentative` | 확정 배치 금지. 안내 문구 또는 후보 정보로만 표시 |
| `unknown` | 확정 배치 금지. 검증 한계 안내 |
| `outdated` | 확정 배치 금지. 재검증 필요 안내 |

`festival_selection` 모드에서는 선택된 축제가 도시 anchor의 근거다. 그러나 해당 축제가 최종 검증에서 `confirmed`가 아니면 일정 블록으로 배치하지 않는다. 이 경우 도시 anchor는 유지하되 `user_notice`에 축제 미배치 사유를 남긴다.
...
| PL-N02 | 정상 | `festival_selection`, confirmed 축제 | 도시 anchor 유지, 축제 블록 배치 가능 |
