...
| 대체 일정 | 월별 기상 경향 또는 데이터 결측에 따른 `alternativeItinerary` 생성 |
| 추천 설명 | 조건 충족, 도시 특성, 일정 가능성 근거를 사용자 언어로 설명 |
| 출력 검증 | 결정적 검증과 의미 검증을 수행하고 실패 시 재작성 또는 안전 폴백 |
| 링크 요청 | 지도 링크와 숙소 검색 링크 생성을 위한 요청 payload 구성 |
...
`anchor_fixed` 모드에서는 Planner 진입 전에 도시가 이미 고정되어 있어야 한다. Planner는 이 anchor를 바꾸지 않는다. (`festival_selection` 모드는 폐지했다 — 축제 포함은 `includeFestivals`로만 제어하며 축제는 도시 선택에 관여하지 않는다. §11 참고.)
...
| `candidate_evidence_package.mode` | `city_discovery`, `anchor_fixed` (`festival_selection` 폐지 — §11) |
| `candidate_evidence_package.selected_city` | 최종 추천 도시. `no_candidate`와 `error`에서는 `null` 가능 |
| `candidate_evidence_package.city_anchor` | 고정 도시 모드의 anchor 근거 |
...
| `user_location` | 이동 부담 설명과 당일치기 밀도 조정 참고 |
| `weather_trends` | 월별 기상 경향 기반 대체 일정 생성 참고 |
...
7. 숙소를 직접 추천하지 않고 `links.staySearch`로 분리한다.
...
| `anchor_fixed` | `city_anchor.locked=true`인 도시를 유지. 후보가 부족해도 다른 도시로 변경하지 않음 |
...
| 3 | 후보 inventory 생성 | primary/reserve 후보를 `attraction`, `restaurant`, `festival`, `placeholder 가능 슬롯`으로 분리 |
| 4 | 슬롯 템플릿 선택 | `tripType` 기준 일차, 시간대, 관광/식사 목표 슬롯 산정 |
| 5 | 테마 coverage 계획 | 필수 테마별 최소 1개 이상 배치 시도 |
| 6 | 축제 overlay 배치 | `includeFestivals=true`이고 `confirmed`인 축제 1개를 Day 1 오후 메인 슬롯에 배치 (§11) |
| 7 | 장소 슬롯 배정 | 관광 슬롯은 테마 round-robin, 식사 슬롯은 restaurant 후보 점수순 배치 |
| 8 | reserve fallback | primary로 부족하면 같은 도시·국가의 reserve 후보 사용 |
| 9 | 대체 일정 생성 | 월별 기상 경향 또는 야외 의존도가 높을 때 생성 |
| 10 | 설명 생성 | 추천 이유, 일정 흐름 이유, 안내 문구 작성 |
| 11 | 결정적 검증 | 필드, 국가, 도시, 테마, 축제, 근거 검증 |
| 12 | 의미 검증 | 설명-일정 일치, 환각, 폴백 안전성 검증 |
...
| tripType | day slots |
| --- | --- |
| `daytrip` | `[attraction, restaurant_lunch, attraction, restaurant_dinner]` |
| `2d1n` | Day1 `[attraction, restaurant_lunch, attraction, restaurant_dinner]`, Day2 `[attraction, restaurant_lunch, attraction]` |
| `3d2n` | Day1 `[attraction, restaurant_lunch, attraction, restaurant_dinner]`, Day2 `[attraction, attraction, restaurant_lunch, attraction, restaurant_dinner]`, Day3 `[attraction, restaurant_lunch, attraction]` |
...
2. 각 후보를 `entity_type` 기준으로 `attraction`, `restaurant`, `festival`로 분리한다.
...
6. 식사 슬롯은 `restaurant` 후보를 점수순 또는 Candidate Evidence 입력 순서대로 배정한다.
7. primary가 부족하면 reserve에서 같은 도시·국가 후보를 보충하고 `fromReserve=true`로 표시한다.
8. 그래도 부족하면 장소명을 만들지 않고 자유시간, 현지 식당 자유 선택, 숙소 체크인 같은 placeholder를 넣는다.
...
현재 데이터에서 `미식·노포` 후보는 대부분 또는 전부 `restaurant` 정보다. 따라서
Planner는 미식 테마를 일반 관광 테마와 같은 attraction bucket에서 경쟁시키지 않고,
meal slot에서 충족되는 테마로 처리한다.

미식 테마 규칙:

1. `theme_code=gourmet_retro` 또는 `assigned_theme=미식·노포`인 restaurant 후보는 lunch/dinner slot에 우선 배치한다.
2. 미식 테마는 restaurant가 배치된 meal slot으로 coverage를 인정한다.
3. `active_required_themes`에 미식과 다른 관광 테마가 함께 있으면, 관광 슬롯 round-robin은 미식을 제외한 테마로 수행하고 미식은 식사 슬롯에서 반영한다.
4. `active_required_themes`가 미식만 있으면, 일정은 "미식 중심"으로 낮은 밀도를 적용한다. 하루 restaurant는 1-2개까지만 배치하고, 남는 attraction slot은 현지 산책·휴식·시장 둘러보기 placeholder로 둔다.
5. restaurant 후보를 attraction slot에 억지로 넣지 않는다. 단, 향후 food market, 양조장, 요리 체험처럼 `entity_type=attraction|experience`로 들어오는 미식 후보는 관광 슬롯 배치가 가능하다.
6. 미식 후보가 부족하면 존재하지 않는 식당명을 만들지 않고 `source=placeholder`, `placeId=null`인 "현지 식당 자유 선택" 블록과 `user_notice`를 생성한다.

미식 coverage 검증 기준:

| 요청 형태 | 충족 기준 |
| --- | --- |
| 미식 + 관광 테마 | 관광 테마는 attraction slot, 미식은 meal slot에서 각각 1회 이상 반영 |
| 미식 단독 daytrip | restaurant meal slot 1개 이상 또는 부족 안내 |
| 미식 단독 숙박형 | 가능한 경우 하루 1개 이상 restaurant meal slot. 후보 부족 시 낮은 confidence와 안내 |
| 미식 후보 0건 | `theme_unmet` 또는 `insufficient_candidates`로 처리하고 명명 식당 생성 금지 |
...
축제는 사용자가 `includeFestivals=true`를 선택했을 때만 일정에 반영한다. 축제 포함은 `includeFestivals`(true/false)로만 제어하며 별도의 `festival_selection` 진입 모드는 사용하지 않는다. 축제는 도시 선택에 관여하지 않는다 — 도시는 Candidate Evidence가 테마·후보로 정하고, 그 선택 도시에 해당 여행 월의 `confirmed` 축제가 있으면 관광 baseline 위에 overlay로 얹는다.
...
`alternativeItinerary`는 일반 일정 수정이 아니라, 같은 목적지 안에서 날씨나 데이터 결측에 대비하는 보조 일정이다.
...
```json
{
  ...
  "alternativeItinerary": {
    ...
    "reason": "해당 월은 야외 일정 변동 가능성이 있어 실내 후보 중심 대체안을 제공합니다.",
    "days": []
  },
...
| 필수 테마 | 관광 테마는 attraction/festival slot, 미식 테마는 meal slot 또는 안내에 반영 | `theme_unmet` |
| 숙박 정책 | 숙소 직접 추천 없이 링크 제공 | `lodging_policy_violation` |
...
현재 API 정본에는 일반 일정 수정 API가 확정되어 있지 않다. 따라서 Planner MVP는 최초 추천 생성과 기상 대체 일정 생성을 우선한다. 일반 수정은 향후 `POST /recommendations/{recommendationId}/revise` 후보 API로 분리 검토한다.
...
| PL-N03 | 정상 | `anchor_fixed`, 후보 충분 | 고정 도시 안에서만 일정 생성 |
...
| PL-W01 | 기상 대체 | 야외 후보 비중 높음 | 동일 도시 실내 중심 `alternativeItinerary` |
| PL-R01 | reserve 사용 | primary 슬롯 배정 부족, reserve 충분 | reserve 사용, `fromReserve=true`, audit 기록 |
| PL-S01 | 슬롯 배정 | `2d1n`, 관광 4개, 식당 3개 | 고정 템플릿에 관광/점심/저녁 슬롯 배정 |
| PL-S02 | 테마 균형 | 필수 테마 3개, 후보 충분 | round-robin으로 테마가 고르게 분산되고 같은 테마 3연속 없음 |
| PL-S03 | 후보 부족 | 식당 후보 없음 | 명명 식당 생성 없이 `source=placeholder` 식사 슬롯과 `user_notice` 생성 |
| PL-G01 | 미식 혼합 | `미식·노포` + `바다·해안` | 바다는 관광 슬롯, 미식은 식사 슬롯에서 충족 |
| PL-G02 | 미식 단독 | `미식·노포`만 있고 후보가 모두 restaurant | 낮은 밀도의 미식 중심 일정, 관광 슬롯은 휴식/산책 placeholder 허용 |
| PL-G03 | 미식 후보 부족 | 미식 필수이나 restaurant 후보 0건 | 명명 식당 생성 금지, `theme_unmet` 또는 부족 안내 |
...
| 일반 일정 수정 API | `POST /recommendations/{recommendationId}/revise` 확정 여부 |
| PlanDraft cache TTL | AgentCore Memory/응답 cache 유지 시간 |
| UI 표시용 source badge | `tour_api`, `official`, `dynamodb`, `web_verified` 등 표준 코드 |
| 기상 대체 일정 trigger | 월별 기상 경향 threshold와 야외 후보 비중 기준 |

## 22. 변경 이력

| 버전 | 날짜 | 작성자 | 변경 내용 |
| --- | --- | --- | --- |
| v1.3 | 2026-06-13 | llm | **무엇을:** 축제 배치를 관광 baseline 위 overlay 모델로 구체화. `festival_selection` 모드 폐지(§3·§4 mode·§7·§20 PL-N02), 축제 포함은 `includeFestivals`만으로 제어하고 축제는 도시 선택에 비관여. §8 단계 6·§9 timeOfDay·§11을 재작성해 선택 도시·여행 월 기준 0/1개, 월 적격성 판정, Day 1 오후 메인 슬롯 배치, 시간 데이터 부재로 timeOfDay 중립화, 밀린 관광지 이동/제거, 실제 개최일 가이드 안내를 정의. 축제 상호작용 규칙으로 ①밀린 관광지가 필수 테마 유일 블록이면 스왑 보존하되 불가하면 축제 우선 배정 후 테마 부족 안내, ②기상 대체 일정(§12)에서 축제 유지 + 야외 리스크 고지, ③다일 축제는 Day 1 단일 블록 + 개최 기간 notice, ④daytrip·미식 단독은 placeholder 자리라 충돌 없음을 추가. **왜:** 축제 데이터는 날짜만 있고 시간이 없으며, 사용자는 여행 월까지만 입력하고 일정은 상대 day로 구성되어 day 단위 겹침·시간대 단정이 불가능하고, `festival_selection` 진입 모드는 실제로 존재하지 않기 때문. `itinerary_flow.md`(v0.4)와 동반 수정 |
