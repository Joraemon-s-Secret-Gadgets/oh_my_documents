...
| 7 | `Itinerary_Planner_Agent` | 일정·대체 일정 구성 | `itinerary`, `alternativeItinerary` |
| 8 | `Explanation_Writer_Agent` | 추천 이유와 동선 설명 작성 | `recommendationReasons`, `itineraryFlowReason`, `user_notice` |
| 9 | `Validation Skill` | 결정적 검증 | 필드·정책 검증 결과 |
| 10 | `Output_Validator_Agent` | 근거성·환각·설명 가능성 의미 검증 | 검증 판정, 실패 카테고리 |
| 11 | `Backend_Serving / SAM` | 응답 패키징, 저장, UI 서빙 | 최종 추천 응답 |
...
| 4 | `itinerary` | Itinerary Planner |
| 5 | `explanation` | Explanation Writer |
| 6 | `validation` | Validation Skill, Output Validator |
...
| 버전 | 날짜 | 작성자 | 변경 내용 |
| --- | --- | --- | --- |
| v0.4 | 2026-06-08 | Codex | Top-K 축제 검증, 후보 0건 폴백, Validator 실패 카테고리, `fulfilled_matrix` 표준 키와 라우팅 우선순위 반영 |
