...
| 4 | `Ranker_Agent` | 구조화 조건 기반 1차 점수화, 축제 검증 대상 Top-K 선별 | 상위 후보 K곳, 점수 근거 |
| 5 | `Festival_Verifier_Agent` | Top-K 후보의 축제 해당 연도 개최일 검증 | 검증 JSON, 캐시 |
| 6 | `Ranker_Agent` | 축제 검증 결과를 반영한 최종 소도시 1곳 선정 | 선정지, 최종 점수 근거 |
| 7 | `Itinerary_Writer_Agent` | 일정·대체 일정·추천 이유·동선 설명을 하나의 구조화 출력으로 생성 | `itinerary`, `alternativeItinerary`, `recommendationReasons`, `itineraryFlowReason`, `user_notice` |
| 8 | `Validation Skill` | 결정적 검증 | 필드·정책 검증 결과 |
| 9 | `Output_Validator_Agent` | 근거성·환각·설명 가능성 의미 검증 | 검증 판정, 실패 카테고리 |
...
Retriever는 S3 vector metadata filter로 국가·도시·테마·월·콘텐츠 유형을 1차 제한하고, AWS Neptune으로 도시·축제·테마·장소 관계를 확장한 뒤 MySQL 원장 또는 DynamoDB 정규화 문서의 확정 필드로 후보를 재검증한다.
...
축제 포함 요청에서는 모든 후보를 웹 검증하지 않고, Ranker의 구조화 1차 점수 상위 K곳(기본 2~3곳)만 검증한다.
...
필수 테마를 충족하는 후보가 0건이면 Ranker는 `no_candidate` 신호를 반환한다.
Supervisor는 이를 검증 실패와 구분해 `validation_retry_count`를 소모하지 않고 `confidence`를 낮춘 뒤 조건 완화 안내 또는 외부 검색 링크 폴백으로 종료한다.

## 7.6 `Itinerary_Writer_Agent`

| 항목 | 내용 |
| --- | --- |
| 책임 | 선정 소도시 기준 일정, 대체 일정, 추천 이유, 일정 흐름 이유, 예외 안내를 하나의 구조화 출력으로 생성 |
| 입력 | 선정지, `tripType`, `active_required_themes`, 축제 검증, 후보 장소, 랭킹 점수 근거, soft/raw 적합도 |
| 출력 | `itinerary`, `alternativeItinerary`, `recommendationReasons`, `itineraryFlowReason`, `externalLinks`, `confidence`, `user_notice` |
| 통합 이유 | 일정과 설명이 같은 후보·점수·검증 결과를 보게 해 "이유-일정" 불일치를 줄이고, 공유 컨텍스트 전달과 LLM 호출을 각각 1회로 줄인다. |
...
| --- | --- |
| `grounding_missing` | `Itinerary_Writer_Agent` 재호출. 동일 선정지와 후보 장소를 유지하되 설명·근거 필드를 우선 재작성한다. |
| `hallucination` | `Polymorphic_Retriever_Agent` 재탐색 또는 해당 항목 제거 |
| `condition_unmet` | Retriever 재탐색 또는 Ranker 재랭킹 |
| `explanation_weak` | `Itinerary_Writer_Agent` 재호출. 일정 골격을 유지하고 설명 필드를 보강한다. |
...
| `Weather Trends Skill` | 월별 기상 경향 정형값 조회 | Retriever, Itinerary Writer |
| `Scoring Skill` | 후보 점수 계산과 랭킹 | Ranker |
| `Matrix Transition Skill` | `fulfilled_matrix` 전이 | Supervisor |
| `Validation Skill` | 결정적 출력 검증 | Itinerary Writer/Validator 앞단 |
| `Link Builder Skill` | 지도·숙소 검색 링크 생성 | Itinerary Writer |
...
`ranking`은 구조화 1차 점수화와 축제 검증 후 최종 선정을 모두 포함하며, 축제 포함 요청에서는 Top-K 산출 후 `festival` 검증을 거쳐 최종 선정까지 완료되어야 `O`로 전이한다.
`generation`은 일정 생성과 설명 생성을 합친 단일 생성 단계다.

| 우선순위 | 키 | 담당 |
| --- | --- | --- |
| 1 | `retrieval` | Retriever |
| 2 | `ranking` | Ranker |
| 3 | `festival` | Festival Verifier |
| 4 | `generation` | Itinerary Writer |
| 5 | `validation` | Validation Skill, Output Validator |
...
| 축제 검증 비용 | 축제 포함 요청은 구조화 1차 랭킹 Top-K만 웹 검증 |
