...
| 일정·설명 생성 | `Itinerary_Writer_Agent` | 여행 기간과 목적지에 맞는 일정, 대체 일정, 추천 이유, 일정 흐름 이유를 하나의 구조화 출력으로 생성 |
...
## 3.6 Itinerary_Writer_Agent

선정된 소도시를 기준으로 여행 기간에 맞는 일정, 대체 일정, 추천 이유, 일정 흐름 이유를 함께 생성하는 Agent다.
당일치기, 1박2일, 2박3일 이상에 따라 일정 밀도와 동선을 다르게 구성하고, 같은 후보·점수·축제 검증 결과를 근거로 설명을 작성한다.

| 항목 | 내용 |
| --- | --- |
| 구현 목적 | 사용자 조건에 맞는 일자별 여행 일정과 설명 가능 응답 생성 |
| 주요 입력 | 선정 소도시, 일정 유형, 축제 검증, 관광지 목록, 랭킹 점수 근거, 사용자 조건 |
| 주요 출력 | 일자별 일정, 이동 흐름, 대체 일정, 추천 이유, 일정 흐름 이유, 안내 문구 |
| 사용 Tool | `Link Builder Skill`, `Weather Monthly Trends Tool` |
| 중요 규칙 | 과도한 이동과 조건 불일치 일정을 만들지 않고, DB 근거 없이 장소나 사실을 새로 만들어내지 않는다 |

## 3.7 Output_Validator_Agent
...
| 6 | `Itinerary_Writer_Agent` | 서비스의 사용자 체감 결과물과 설명 가능성 확보 |
| 7 | `Output_Validator_Agent` | 환각과 조건 불일치 방지 |
| 8 | `Backend_Serving / SAM` | 검증 완료 결과를 저장하고 UI 서빙 형태로 제공 |
...
- `Itinerary_Writer_Agent`는 `confirmed` 축제만 일정에 직접 배치하고, `tentative` 축제는 안내 문구 또는 후보 정보로만 사용한다.
