...
| 수정안 | `agent_build_target.md`의 `_Agent` 접미사 표기(`Itinerary_Planner_Agent`, `Explanation_Writer_Agent`, `Output_Validator_Agent`)를 정본으로 채택하고, `05_agent_spec.md` 전체를 일괄 통일 |
...
| `Itinerary+Explanation`(통합, 6.5) | 생성형 일정·설명 | 상위 |
...
| `Itinerary+Explanation`(통합) | Claude Sonnet (3.00/15.00) | Llama 3.1 405B (0.65/0.80) |
...
| **Itinerary+Explanation(통합)** | Claude Sonnet | 3.0 | 2.5 | 0.0465 |
...
전파 필요(승인 시): 다이어그램(3.1/3.2 및 5장), `05_agent_spec.md` 6.7·6.8 노드 통합, `agent_harness_design.md` 노드 단위 테스트·토큰 회귀.
