...
| Destination Search Tool | Candidate Evidence | 축제 city seed 조회, 관광지 후보 검색 | `recommended_places`, `reserve_places`, `festival_candidates` |
...
→ 사용자 travel theme OR 조건으로 축제 후보 조회
...
다만 축제 포함 city discovery에서는 그 전에 Candidate Evidence가 `festival.month == travelMonth`와 사용자 테마 OR 조건으로 축제 city seed를 먼저 만든다.
