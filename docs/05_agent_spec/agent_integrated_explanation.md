...
| `themes` | `["해안/섬", "미식/노포"]` | 사용자가 선택한 테마 |
| `entryType` | `city_discovery`, `destination_anchor`, `festival_selection` | 추천 진입 방식 |
| `destinationId` | `...` | 특정 도시/장소에서 시작할 때의 기준 ID |
| `includeFestivals` | `true`, `false` | 축제 포함 여부 |
| `user_location` | 위도/경도 또는 지역명 | 이동 거리 계산에 활용 가능한 사용자 위치 |
...
| `alternativeItinerary` | 비나 날씨 변수, 후보 부족 상황 등을 고려한 대안 일정 |
...
| `externalLinks` | 지도, 숙소 검색 등 외부 연결 링크 |
| `confidence` | 추천 신뢰도 |
...
| Tool/Skill | 역할 |
| --- | --- |
| Destination Search Tool | 벡터 검색, 테마 필터, 장소/도시 후보 조회 |
...
사용자의 자연어, UI 선택값, 온보딩 선호, 이전 대화 요약을 모아서 **추천 파이프라인이 이해할 수 있는 구조화된 입력**으로 바꾼다.
...
예를 들어 UI에서 `country=KR`을 선택했는데 문장에 "일본도 괜찮아"라고 쓰면, 현재 턴이 더 구체적인 변경 요청인지 판단해야 한다. 불명확하면 바로 추측하지 않고 확인 질문을 한다.
...
```json
{
  ...
  "tripType": "2d1n",
  "destinationId": null,
  "active_required_themes": ["해안/섬", "미식/노포"],
...
| 필드 | 의미 | 예시 |
| --- | --- | --- |
| `cleaned_raw_query` | 검색에 직접 쓸 핵심 요청 | "바다 노포 맛집 1박2일 여행지" |
| `soft_preference_query` | 분위기, 취향, 감성 조건 | "조용한, 덜 붐비는, 여유로운" |

이렇게 나누는 이유는, 모든 문장 표현이 같은 성격의 조건은 아니기 때문이다.

예를 들어 "바다"와 "노포 맛집"은 장소 후보를 찾는 데 직접적인 조건이다.  
반면 "조용한", "덜 붐비는", "감성적인"은 후보의 성격을 판단하는 보조 신호에 가깝다.

Candidate Evidence Agent는 이 두 쿼리를 모두 검색에 사용하되, 서로 다른 근거 채널로 다룬다.
...
| `fulfilled_matrix.festival` | 축제 포함 요청이 있으면 Festival Verifier Agent 실행 |
...
| `active_required_themes` | 반드시 고려해야 하는 테마 |
...
flowchart TD
    ...
    B --> D["테마별 raw vector search"]
    C --> E["테마별 soft vector search"]
    ...
    L --> M["DynamoDB 상세 정보 rehydrate"]
    M --> N["Candidate Evidence Package"]
```
...
Candidate Evidence Agent가 후보를 찾을 때 사용하는 핵심 Tool이다.

| 기능 | 설명 |
| --- | --- |
| 벡터 검색 | raw query와 soft query에 가까운 장소 후보 검색 |
| 테마 필터 | 사용자가 요구한 테마별 후보 제한 |
| 메타데이터 필터 | 국가, 도시, content type 등 조건 반영 |
| 장소 상세 조회 | 최종 후보의 이름, 좌표, 설명, 이미지 등 복원 |

Lovv는 S3 Vector Index를 통해 후보를 빠르게 찾고, 필요한 경우 DynamoDB 등 원본 저장소에서 상세 정보를 다시 가져온다.
...
| `theme_coverage_score` | 필수 테마를 얼마나 잘 포함하는지 |
...
| theme match | 필수 테마와의 일치도 |
| source quality | 후보 데이터가 설명/이미지/좌표 등 충분한 정보를 갖는지 |
| local distance penalty | 같은 도시 안에서 동선상 너무 불리한지 |

### 7.7 Theme Quota와 Reserve

추천 후보를 만들 때 특정 테마가 결과를 독점하면 안 된다.  
예를 들어 사용자가 `해안/섬`과 `미식/노포`를 함께 요구했는데, 해안 명소만 잔뜩 나오면 실제 요청을 만족하지 못한다.

그래서 Candidate Evidence Agent는 primary 후보를 구성할 때 테마별 최소 할당량과 완화 가능한 최대 할당량을 둔다.

| 개념 | 설명 |
| --- | --- |
| primary 후보 | Planner가 우선 사용하는 장소 후보 |
| reserve 후보 | 후보 부족이나 일정 슬롯 보완에 쓰는 예비 후보 |
| min quota | 각 필수 테마가 최소한 포함되어야 하는 후보 수 |
| soft max quota | 한 테마가 primary 후보를 과도하게 차지하지 않게 하는 완화 가능한 상한 |
...
| `no_candidate` | 조건을 만족하는 후보가 없음 | 일정 생성하지 않고 조건 완화 안내 |
...
사용자가 축제를 포함해 달라고 했거나, 축제 기반 추천으로 진입한 경우 Festival Verifier Agent가 목표 연도와 월에 맞는지 별도로 검증한다.
...
| 일반 장소 추천과 분리 | 축제는 별도 검증 결과를 Planner에 전달 |
...
| `mode` | 도시 탐색, 고정 도시, 축제 선택 등 |
| `selected_city` | 최종 추천 대상 도시 |
| `recommended_places` | 우선 사용할 장소 후보 |
| `reserve_places` | 부족할 때 사용할 예비 장소 |
...
| Day 1 | 점심 | `미식/노포` 식당 후보 |
| Day 1 | 오후 | 핵심 테마 명소 |
| Day 1 | 저녁 | 식당 또는 야간 산책 |
...
4. placeholder는 실제 장소처럼 꾸미지 않고, "자유 시간", "현지 식당 선택"처럼 안전하게 표현한다.

### 9.4 `미식/노포` 테마의 특별 처리

`미식/노포`는 일반 관광지 테마와 다르게 다룬다.

현재 데이터 구조상 `미식/노포` 후보는 주로 음식점이다.  
따라서 Planner는 이 테마를 관광 명소 슬롯이 아니라 식사 슬롯에 우선 배치한다.

| 상황 | 처리 |
| --- | --- |
| `미식/노포` 후보가 충분함 | 점심/저녁 슬롯에 식당 후보 배치 |
| 미식 후보만 많고 관광지가 부족함 | 낮은 밀도의 미식 중심 일정 구성 |
| 미식 후보가 없음 | 식당 이름을 지어내지 않고 부족 안내 |
| 관광지와 미식이 함께 있음 | 관광 슬롯과 식사 슬롯을 분리해 구성 |

이 처리가 없으면 음식점이 관광 명소처럼 배치되거나, 반대로 사용자가 원한 미식 조건이 일정에서 사라질 수 있다.
...
Planner는 가능한 경우 `alternativeItinerary`도 만든다.

대안 일정은 다음 상황에서 유용하다.

| 상황 | 대안 일정 방향 |
| --- | --- |
| 비나 날씨 변수 | 실내, 문화, 공방, 카페, 식당 중심 |
...
| Destination Search Tool | Candidate Evidence | 도시/장소 후보 검색 | `recommended_places`, `reserve_places` |
| S3 Vector Search | Candidate Evidence | raw/soft query 기반 의미 검색 | 후보 수집, semantic score |
| DynamoDB Rehydrate | Candidate Evidence | primary 후보 상세 정보 복원 | Planner가 사용할 장소 상세 |
...
| Link Builder Skill | Planner/Backend | 지도/숙소 검색 링크 생성 | `externalLinks` |
...
1. `해안/섬` 테마 후보를 raw/soft query로 검색한다.
2. `미식/노포` 테마 후보를 raw/soft query로 검색한다.
3. 후보를 도시별로 묶는다.
4. 두 테마를 모두 만족할 수 있는 도시를 우선 본다.
5. 도시 점수와 장소 점수를 계산한다.
6. 최종 추천 도시와 primary/reserve 후보를 만든다.
...
| `recommended_places` | 해안 명소, 전망지, 노포 식당 후보 등 |
| `reserve_places` | 예비 식당, 실내 장소, 산책 장소 등 |
| `coverage_audit` | 해안/섬 후보와 미식/노포 후보가 primary에 포함됐는지 기록 |
...
| 주의 사항 | 실시간 혼잡도나 식당 영업 여부는 별도 확인 필요 |
| 신뢰도 | 후보 충분성과 검증 결과에 따라 high/medium/low |
...
대부분의 경우 아니다.  
현재 데이터에서 `미식/노포`는 음식점 성격이 강하므로, Planner는 이를 점심/저녁 같은 식사 슬롯에 우선 배치한다.

### 15.5 "축제는 장소 후보와 동일하게 다루나요?"

아니다.  
축제는 날짜 검증이 중요하므로 Festival Verifier Agent를 통해 별도로 확인한다. 확인되지 않은 축제는 확정 일정에 넣지 않는다.
...
> Lovv의 Agent는 사용자의 문장을 바로 일정으로 바꾸는 단일 모델 호출이 아니라, Intent Agent가 입력 조건을 구조화하고, Candidate Evidence Agent가 데이터 기반으로 도시와 장소 후보를 찾고 점수화한 뒤, Planner Agent가 그 후보만 사용해 일정과 설명을 만들고 검증하는 파이프라인입니다. Supervisor는 각 단계의 상태를 보고 다음 Agent를 실행하며, 축제처럼 날짜 검증이 중요한 정보는 별도 Verifier를 거칩니다. 최종적으로 Backend가 검증된 일정, 추천 이유, 신뢰도, 주의 사항, 외부 링크를 API 응답으로 포장합니다.

좀 더 자세히 3분 정도 설명한다면 다음 순서가 좋다.

1. 사용자는 자연어와 UI 선택으로 여행 조건을 입력한다.
2. Intent Agent는 조건을 `country`, `travelMonth`, `tripType`, `themes`, `raw query`, `soft preference`로 정리한다.
3. Supervisor는 후보 근거가 필요하다고 판단해 Candidate Evidence Agent를 실행한다.
4. Candidate Evidence Agent는 벡터 검색과 점수화로 추천 도시와 primary/reserve 장소 후보를 만든다.
5. 축제가 필요한 경우 Festival Verifier가 목표 연도 날짜를 검증한다.
6. Planner Agent는 후보만 사용해 일정, 대안 일정, 추천 이유, 주의 사항을 만든다.
...
| 도시/장소 후보 근거 수집 | Candidate Evidence Agent |
