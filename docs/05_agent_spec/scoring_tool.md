...
| 입력 | 검색 후보 dict, active themes, 후보 기준 좌표, 사용자 위치, 혼잡도 index |
...
`score_place(cand, active_themes, reference_location=None)`는 단일 장소 후보의 점수를 계산한다.
...
| `theme_match_score` | active theme와 후보 `theme_tags`가 겹치면 `+0.2` | 필수 테마 직접 일치 보너스 |
...
복수 active theme가 있을 때 `theme_balance`는 Shannon entropy를 사용한다.
...
| active theme 없음 | theme coverage 0, theme balance 기본값 유지 |
