...
| 2 | rich embedding_text: 장소명·도시명·테마·유형·주소·메뉴·원본 설명·파생 태그 (§9) | §5.2 chunk 템플릿이 대부분 포함. 단 **restaurant 템플릿에 주소 누락** | **공백** | FR-03에서 restaurant chunk에 주소 추가 |
...
- **restaurant chunk 템플릿에 주소를 추가한다** (대조표 #2 공백 해소). 이 변경은 `s3_vector_index_plan.md` §5.2에도 반영한다.
