...
> **[v0.2 재구성]** 취득 단계는 이미 `tour-api-korea` repo에 구현되어 있으므로, 본 PRD는 취득을 **외부 업스트림 의존(참조)**으로 두고 그 산출물을 핸드오프 계약(§3.5)으로 받는다. 신규 코드 작업 범위는 §1.3 "구현 대상"으로 한정한다.
...
| tel 폴백·방문통계 필드명 | 전처리 보강 / `locals_*`→`local_*` 매핑 | 전처리에서 보강·표준명 매핑 (OI-3) |
...
계약 불변식(파이프라인이 가정하는 것): 관광지/축제는 `contentid` 안정 키 보유, City는 `KR-{GW|GB}-{CITY_EN}` 형식, 축제 기간은 `detail.intro.eventstartdate/eventenddate`(YYYYMMDD), 방문통계는 `monthly_statistics[]` 12개월·`touDivCd` 1/2/3 합산 일평균. **계약이 깨지면(필드명·구조 변경) repo와 본 PRD §3.5를 동시에 갱신한다.**
...
- **FR-8 연락처 폴백**: `tel`은 코드 미출력이므로 전처리에서 `detail.intro`의 `infocenter`/`infocenterculture`/`infocenterfood`/`sponsor1tel`을 우선순위로 폴백한다(없으면 생략).
- **FR-9 테마 검증**: `lclsSystm3 or cat3` 기준 6대 테마 매핑 무결성 검증 + 축제 오버라이드 46건 반영. 미매핑은 `content_review`.
...
| 단계 | 산출물 | 완료 기준 |
| --- | --- | --- |
| M0 결정 확정 | §3.4 Pre-req 4건 합의 | City ID·테이블·tel·수량 정본 확정 |
...
| ID | 항목 | 상태 | 신뢰도 |
| --- | --- | --- | --- |
| OI-1 | 테마 합계 4,911 ↔ 적재 3,709 차이(1,202건) | 정본 3,709, Transform 수량 재집계로 원인 규명 권고 | 확인 필요 |
| OI-2 | City ID 도 코드 정책(영문 `GW`/`GB` vs 숫자 `42`/`47`) | 미정 — 정책 결정 필요 | 확인 필요 |
| OI-3 | tel 폴백·`prefecture_id`·City 설명/기후/좌표 보강 | 코드 미구현 → 전처리/후속 수집 과제 | 확인 필요 |
| OI-4 | 적재 테이블 명칭 정합(`TourKoreaData` 단일 테이블 ↔ 전처리 문서 `LovvCity` 등 후보) | DB 설계(04)를 정본으로 통일 권고 | 중 |
| OI-5 | 일본(JP) 파이프라인 | 본 PRD 범위 외, 별도 설계 | — |
