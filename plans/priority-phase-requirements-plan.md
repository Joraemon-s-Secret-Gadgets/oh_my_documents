...
- P5 항목은 삭제하지 않고 `이번 프로젝트 제외 / 향후 검토` 상태로 남겨 후속 기획에서 추적 가능하게 한다.
...
- [x] `FR-ASTRO-*`, 웰니스, 예산 역산은 P5 그룹으로 정렬된다.
...
|---|---|---|
| 기존 요구사항에 P5 기능이 이미 포함되어 있어 범위가 혼란스러울 수 있음 | Medium | 삭제 대신 `향후 검토` 상태로 명확히 표시 |
...
- P5 제외 항목은 현재 일반 표 안의 `제외/검토` 상태값으로 표시한다.
