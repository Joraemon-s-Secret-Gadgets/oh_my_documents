...
`docs/06_database_design/06_database_design.md`를 대표 문서로 두고, 로브(Lovv) 서비스의 데이터베이스 설계를 개념 설계, 논리 설계, 물리 설계 순서로 작성한다.
현재 DBMS 방향, PoC DB 설계 초안, 통합 설계 초안, NoSQL 스키마 설계 내용을 정리해 하나의 일관된 Markdown 설계 문서로 반영한다.

## 대상 파일

| 구분 | 파일 | 용도 |
| --- | --- | --- |
| 대표 문서 | `docs/06_database_design/06_database_design.md` | 최종 DB 설계 명세서 본문 |
| RDB 초안 | `docs/06_database_design/lovv-poc-database-design.md` | PoC 기준 MySQL 테이블, 관계, JSON 컬럼 참고 |
| 통합 초안 | `docs/06_database_design/integrated_draft.md` | 전체 도메인, ERD, 인덱스, 보존 정책 참고 |
| NoSQL 초안 | `docs/06_database_design/nosql_schema_design.md` | DynamoDB 또는 NoSQL 데이터 구조 참고 |
| DBMS 결정 | `docs/06_database_design/DBMS.md` | MySQL, DynamoDB, VectorDB 방향 확인 |
| 생성물 | `pages/06_database_design.html` | Markdown 반영 후 재생성 대상 |
...
- [x] `docs/07_agent_spec/07_agent_spec.md`에서 Agent 실행 상태, 로그, RAG 데이터 저장 요구를 확인한다.
...
- [x] `docs/06_database_design/06_database_design.md`를 갱신한다.
- [x] Markdown 원본 기준으로 `pages/06_database_design.html`을 재생성한다.
...
- 대표 문서와 보조 초안의 DBMS 결정이 `MySQL 8 LTS`, `AWS DynamoDB`, `VectorDB` 기준으로 일관되는지 확인한다.
- 생성된 HTML의 목차와 본문 섹션이 Markdown 구조와 대응하는지 확인한다.
- 관계형 데이터, 로그성 데이터, 벡터 검색 데이터가 서로 다른 저장소 책임으로 분리되어 있는지 확인한다.

## 완료 기준

- `docs/06_database_design/06_database_design.md`에 개념 설계, 논리 설계, 물리 설계가 모두 포함된다.
- 각 설계 단계가 같은 엔티티와 저장소 책임을 기준으로 이어진다.
- `pages/06_database_design.html`이 대표 Markdown과 같은 내용을 제공한다.
